...
# 1. 프론트엔드 통신 데이터 규격 정의
class UserInput(BaseModel):
    player_count: str        # 인원수 (2인, 3~4인, 5인 이상)
    playing_time: str        # 플레이 시간
    difficulty: str          # 난이도 (웨이트)
    atmosphere: str          # 모임의 성격
    mechanism: str           # 핵심 메커니즘

class GameDetail(BaseModel):
    book_title: str          # 과제 규격 맞춤용 (보드게임 이름)
    score: int               # 매칭 스코어 (%)
    catchphrase: str         # 한 줄 요약
    tags: List[str]          # 게임 정보 태그
    recommended_for: str     # 이런 분들에게 추천해요
    setup_info: str          # 준비 및 난이도 체감
    rules: List[str]         # 3줄 핵심 룰 요약

class RecommendResponse(BaseModel):
    games: List[GameDetail]

# 2. 정밀 매칭을 위한 보드게임 데이터베이스 (DB) 구축
BOARDGAME_DB = [
    {
        "name": "루미큐브",
        "best_players": "3~4인", "time": "적당하게 (30분~1시간)", "difficulty": "초보자용 (룰이 5분 컷)",
    ...
    {
        "name": "할리갈리",
        "best_players": "3~4인", "time": "짧고 굵게 (30분 미만)", "difficulty": "초보자용 (룰이 5분 컷)",
    ...
    {
        "name": "젠가",
        "best_players": "2인", "time": "짧고 굵게 (30분 미만)", "difficulty": "초보자용 (룰이 5분 컷)",
    ...
    {
    "name": "5초 준다!",
    "best_players": "3~6인", "time": "짧고 굵게 (30분 미만)", "difficulty": "초보자용 (룰이 5분 컷)",
...
{
    "name": "달무티",
    "best_players": "5~8인", "time": "적당하게 (30분~1시간)", "difficulty": "초보자용 (룰이 5분 컷)",
...
{
    "name": "코드네임",
    "best_players": "4~8인", "time": "적당하게 (30분~1시간)", "difficulty": "초보자용 (룰이 5분 컷)",
...
{
    "name": "꼬치의 달인",
    "best_players": "2~4인", "time": "짧고 굵게 (30분 미만)", "difficulty": "초보자용 (룰이 5분 컷)",
...
{
    "name": "스파이폴",
    "best_players": "4~8인", "time": "짧고 굵게 (30분 미만)", "difficulty": "초보자용 (룰이 5분 컷)",
...
{
    "name": "카르카손",
    "best_players": "2~4인", "time": "적당하게 (30분~1시간)", "difficulty": "초보자용 (룰이 5분 컷)",
...
def get_recommendation(data: UserInput):
    ...
    for game in BOARDGAME_DB:
        ...
        # 가중치 알고리즘 매칭 규칙 설계 (수연님의 기획 그대로 반영)
        ...
        # 최하 매칭 점수 방어 및 랜덤 편차 부여 (개인화 스코어 느낌 연출)
        score_percentage = int((match_points / total_criteria) * 100)
        if score_percentage == 0: score_percentage = 45  # 기본 베이스 점수
        elif score_percentage == 100: score_percentage = 98 # 찰떡 점수 최고점 연출
        else: score_percentage = min(score_percentage + 15, 92) # 매칭 보정
            
        scored_games.append({
            "book_title": game["name"], # 프론트 엔드 규격과 맞춤
