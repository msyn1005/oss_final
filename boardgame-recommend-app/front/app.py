...
st.title("🎲 오늘 모임의 인생 보드게임 매칭 엔진")
st.caption("인원수, 플레이 시간, 성향을 분석하여 오늘 모임 분위기를 하드캐리할 TOP 3 보드게임을 추천합니다.")
st.markdown("---")

# 2. UX 흐름을 반영한 입력 컴포넌트 구성 (백엔드 새 데이터와 100% 일치)
st.subheader("⚙️ 1. 필수 조건 입력")
col1, col2 = st.columns(2)

with col1:
    # 백엔드의 'best_players' 규격에 맞게 매칭
...
st.markdown(" ")
st.subheader("🎯 2. 취향 및 모임 성향 필터링")

# 백엔드의 'difficulty' 규격에 맞게 매칭
difficulty = st.radio(
    "보드게임 숙련도 (난이도)를 선택하세요", 
    ["초보자용 (룰이 5분 컷)", "중급자용 (약간의 전략 필요)", "고인물용 (설명만 30분)"]
)

col3, col4 = st.columns(2)
with col3:
    # 백엔드의 'atmosphere' 규격에 맞게 매칭
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if not result.get("games"):
                ...
                # 4. TOP 3 보드게임을 카드 및 상세 페이지 형태로 순서대로 출력
                for idx, game in enumerate(result["games"]):
                    ...
                    with st.container():
                        ...
                        # 태그 노출 (인원, 시간, 난이도를 칩 형태로 예쁘게 표현)
                        tags_str = " ｜ ".join([f"`{tag}`" for tag in game["tags"]])
                        st.markdown(tags_str)
                        
                        # 상세 보기 확장 컴포넌트 (사용자가 결정을 내리게 만드는 치트키 영역)
                        with st.expander("🔍 상세 정보 및 3줄 핵심 룰 보기"):
                            st.markdown(f"🎯 **이런 분들에게 추천해요:**\n\n{game['recommended_for']}")
                            st.markdown(f"⏱️ **체감 장벽 데이터:**\n\n{game['setup_info']}")
                            
                            st.markdown(" ")
                            st.markdown("**💡 3줄 핵심 룰 요약**")
